...
public class TileEntityBlacksmithSmelter extends TileEntity implements ITickable, IInventory {
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.blacksmithsmelter");
	}
	...
	public static boolean isBurning(TileEntityBlacksmithSmelter te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			BlockBlacksmithSmelter.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = RecipesBlacksmithSmelter.getInstance().getSmelterResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = RecipesBlacksmithSmelter.getInstance().getSmelterResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
